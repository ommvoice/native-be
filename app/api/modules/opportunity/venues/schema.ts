...
import { OPPORTUNITY_ROUTE_DOG_FACILITY_SLUGS } from "../routes/constants/dog-facility.js";
...
const optionalTerrainTypeSlug = yup
  .string()
  .trim()
  .optional()
  .nullable()
  ...
  .test(
    "known-terrain-type",
    "terrainTypeSlug must be a known terrain type",
    (v) =>
      v == null ||
      OPPORTUNITY_TERRAIN_SLUGS.includes(
        v as (typeof OPPORTUNITY_TERRAIN_SLUGS)[number],
      ),
  );
...
export const createOpportunityVenueSchema = yup.object({
  ...
  openingDaysAndTimes: yup.string().trim().optional().nullable(),
  openingExclusions: yup.string().trim().optional().nullable(),
  activityGroupSlug: optionalActivityGroupSlug,
  terrainTypeSlug: optionalTerrainTypeSlug,
  ...
  estimatedTimeToSpend: yup.string().trim().optional().nullable(),
...
export type CreateOpportunityVenueBody = yup.InferType<
  typeof createOpportunityVenueSchema
>;

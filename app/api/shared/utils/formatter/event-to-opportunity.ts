...
function resolveMinAge(data: OpportunityVenueV2Response): number | null {
...
function resolveMaxAge(data: OpportunityVenueV2Response): number | null {
...
function resolveSuitableFor(data: OpportunityVenueV2Response): string[] | null {
  const bands: string[] = [];
...
function buildFacilities(data: OpportunityVenueV2Response): string[] | null {
  const all = [data.eventGeneralFacilities, data.eventChildFacilities, data.eventAdultFacilities]
    .flatMap((raw) => splitList(raw) ?? []);
  return all.length > 0 ? all : null;
}

function buildEventTimes(data: OpportunityVenueV2Response): Record<string, string[]> | null {
...
function resolvePriceInfo(data: OpportunityVenueV2Response): string | null {
  const parts: string[] = [];
...
export const eventToOpportunity = (data: OpportunityVenueV2Response): OpportunityDetail => {

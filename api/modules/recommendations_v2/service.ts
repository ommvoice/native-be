...
export class RecommendationsV2Service {
  ...
  /** Parent interests plus a single child (ages, interests for that child only). */
  ...
  private scoreCandidateFull(
    ...
    // Three criteria items: interests, age, distance. Each scored 0–100, then combined into a single score with configurable weights (currently all equal weight, but could be tweaked in future or even made dynamic per family).

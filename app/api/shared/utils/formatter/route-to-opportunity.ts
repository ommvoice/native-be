...
function resolveMinAge(data: OpportunityRouteV2Response): number | null {
...
function resolveMaxAge(data: OpportunityRouteV2Response): number | null {
...
function resolveSuitableFor(data: OpportunityRouteV2Response): string[] | null {
  const bands: string[] = [];
...
function buildFacilities(data: OpportunityRouteV2Response): string[] | null {
...
export const routeToOpportunity = (data: OpportunityRouteV2Response): OpportunityDetail => {
  return {
    // ── Core ──────────────────────────────────────────────
    id: data.id,
    opp_type: "route",
